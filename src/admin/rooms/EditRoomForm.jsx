...
const EditRoomForm = ({ roomId, onRoomUpdated }) => {
  ...
  useEffect(() => {
    const fetchRoom = async () => {
      try {
        ...
        const response = await UserService.getRoom(token, roomId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await UserService.updateRoom(token, roomId, room);
  ...
  return (
    <div className="form-container-room max-w-xl mx-auto p-4 space-y-4 bg-white text-dark-grey shadow-md rounded h-full overflow-auto">
      <h2 className="text-lg font-semibold text-gray-800 mb-6 text-center">Edit Room</h2>
